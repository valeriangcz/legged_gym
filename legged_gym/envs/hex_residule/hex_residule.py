...
from legged_gym.envs.hex_residule.hex_residule_config import HexResiduleCfg
...
import math
import time
class HexResidule(LeggedRobot):
    def __init__(self,cfg:HexResiduleCfg,sim_params,physics_engine,sim_device,headless):
        super().__init__(cfg,sim_params,physics_engine,sim_device,headless)
        self.cfg:HexResiduleCfg = cfg
    ...
    def step(self,actions):
        ...
        for _ in range(self.cfg.control.decimation):
            self.expert_actions=self.get_expert_actions()
            mix_actions = self.expert_actions + actions*self.cfg.control.action_scale
            # print("actions\n",actions[0]*self.cfg.control.action_scale)
            
            self.torques = self._compute_torques(mix_actions).view(self.torques.shape)
    ...
    def get_expert_actions(self):
        ...
        self.expert_actions =  self.expert.ProcessCommand(command,
                                                          self.dof_pos,self.dof_vel,
                                                          self.contact_forces[:,self.feet_indices,2]) #此时的actions还是关节角度的绝对位置，要进行转化
    ...
    def _reward_footend_pos_xy(self):
        ...
        return torch.exp(-xy_dist/(0.2*0.5))
...
if __name__ == '__main__':
    ...
    env = HexResidule(cfg,sim_params,args.physics_engine,args.sim_device,args.headless)
