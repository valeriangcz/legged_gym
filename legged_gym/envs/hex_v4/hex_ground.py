...
class HexGround(LeggedRobot):
    ...
    def step(self,actions):
        ...
        for i in range(self.cfg.control.decimation):
            ...
            # print("torqures[0,2]=",self.torques[0,2],"torques_real[0,2]=",self.torques_real[0,2])
    ...
    def _compute_torques(self, actions):
        ...
        #不同关节角的力矩限制不一样
        # torques = self.actuator.get_torques(pos_err,vel_err) #env_nums*18
        # torques_ = torques.view(self.num_envs,6,3)
        # torques_[...,1]=-27.0
        # torques_[...,2]=-27.0
        # # torques_[...,2] = torch.clip(torques_[...,2],min=-5,max=5)
        # # torques_[...,0:2] = torch.clip(torques_[...,0:2],min=-5,max=5)
        # torques = torques_.view(self.num_envs,18)
        ...
        # print(f"pos_err={pos_err[0,2]}, vel_err={vel_err[0,2]}, torques={torques[0,2]}")
        # if torch.abs(torques[0,2]) == 27.0:
        #     exit(0)
    ...
    def post_physics_step(self):
        ...
        # print("-------------self.dof_vel=",self.dof_vel[0,2])
    ...
    def get_expert_actions(self):
        ...
        expert_dofs = self.expert.ProcessCommand(command,self.dof_pos,self.dof_vel,self.contact_forces[:,self.feet_indices,2]) #此时的actions还是关节角度的绝对位置，要进行转化
    ...
    def compute_observations(self):
        ...
        #[quat(4), ang_vel(3), lin_acc(3), last_action(18), dof_pos(18), dof_vel(18), commands(3)] = 67
        self.obs_buf = torch.cat([self.base_quat*self.obs_scales.quat,
                                  self.base_ang_vel*self.obs_scales.ang_vel,
                                  self.base_lin_acc*self.obs_scales.lin_acc,
                                  self.last_actions*self.obs_scales.actions,
                                  (self.dof_pos-self.default_dof_pos)*self.obs_scales.dof_pos,
                                  self.dof_vel*self.obs_scales.dof_vel,
                                  self.commands*self.commands_scale],dim=-1)                 
        if self.add_noise:
    ...
    def _init_buffers(self):
        ...
        # self.torques_real = gymtorch.wrap_tensor(_forces).view(self.num_envs,-1)
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[10:28] = 0.0
        noise_vec[28:46] = noise_level * noise_scales.dof_pos*self.obs_scales.dof_pos
        noise_vec[46:64] = noise_level * noise_scales.dof_vel * self.obs_scales.dof_vel
        noise_vec[64:67] = 0.0 #command
        #[last_action(18), dof_pos(18), dof_vel(18), dof_torque(18), command(3)]
        # noise_vec[:18]=0.0
        ...
        # noise_vec[72:75]=0.0
        #[last_action(18), dof_pos(18), dof_vel(18), command(3), measured_hegiths(143)]
        #[quat(4), ang_vel(3), lin_acc(3), last_action(18), dof_pos(18), dof_vel(18), torques(18), commands(3)] = 85
        # noise_vec[:4] = noise_level * noise_scales.quat * self.obs_scales.quat
        # noise_vec[4:7] = noise_level * noise_scales.ang_vel * self.obs_scales.ang_vel
        # noise_vec[7:10] = noise_level * noise_scales.lin_acc * self.obs_scales.lin_acc
        # noise_vec[10:28] = 0.0
        # noise_vec[28:46] = noise_level * noise_scales.dof_pos * self.obs_scales.dof_pos
        # noise_vec[46:64] = noise_level * noise_scales.dof_vel * self.obs_scales.dof_vel
        # noise_vec[64:82] = noise_level * noise_scales.dof_torque * self.obs_scales.dof_torque
        # noise_vec[82:85] = 0.0 #command
        ...
        if self.privileged_obs_buf is not None:
            ...
            # noise_vec[85:88] = noise_level * noise_scales.lin_vel * self.obs_scales.lin_vel
            # noise_vec[88:91] = noise_level * noise_scales.gravity * self.obs_scales.gravity
            # noise_vec[91:97] = noise_level * noise_scales.contact_force * self.obs_scales.contact_force
    ...
    def _reward_footend_pos_xy(self):
        ...
        # 将奖励改为只要thigh关节超过0点向更远的一方，就有奖励，可以通过设置0来改变奖励的动作范围
        return torch.exp(-xy_dist/(0.6*0.5))*(torch.norm(self.commands[:,:3],dim=1)>0.2)
    ...
    def _reward_feet_contact_forces_increase(self):
        ...
        return feet_force_dt.sum(dim=1) *( torch.norm(self.commands[:,:3],dim=1)>0.2)
    ...
    def _reward_thigh_angle(self):
        #边角的thigh关节
        thigh = self.dof_pos.view(self.num_envs,6,3)[...,0] #N*6
        #LB LF LM RB RF RM
        #找到RF和LB，小于-0.1就惩罚 找到LF和RB，大于0.1就惩罚
        # print("thigh angles=",thigh[0])

        return torch.sum(thigh[:,[0,4]]<-0.1,dim=1) + torch.sum(thigh[:,[1,3]]>0.1,dim=1)
